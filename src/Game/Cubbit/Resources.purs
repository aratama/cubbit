module Game.Cubbit.Resources (Resources, loadResources, loadResourcesH, resourceCount) where
...
import Game.Cubbit.Sounds (Sounds, loadSounds, loadSoundsH)
...
loadResourcesH canvasGL inc = do
    ...
    Options options <- liftAff $ case runExcept (readOptions response.response) of
        Left err -> throwError (error (show err))
        Right opt -> pure opt
